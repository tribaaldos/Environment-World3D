...
import { quat, RigidBody, CapsuleCollider, useRapier, CylinderCollider, } from "@react-three/rapier";
...
const Ecctrl = forwardRef(({ children, debug = false, capsuleHalfHeight = 0.35, capsuleRadius = 0.3, floatHeight = 0.3, characterInitDir = 0, // in rad
followLight = false, disableFollowCam = false, disableFollowCamPos = { x: 0, y: 0, z: -5 }, disableFollowCamTarget = { x: 0, y: 0, z: 0 }, 
// Follow camera setups
camInitDis = -5, camMaxDis = -7, camMinDis = -0.7, camInitDir = { x: 0, y: 0, z: 0 }, // in rad
camTargetPos = { x: 0, y: 0, z: 0 }, camMoveSpeed = 1, camZoomSpeed = 1, camCollision = true, camCollisionOffset = 0.7, 
// Follow light setups
followLightPos = { x: 20, y: 30, z: 10 }, 
// Base control setups
maxVelLimit = 2.5, turnVelMultiplier = 0.2, turnSpeed = 15, sprintMult = 2, jumpVel = 4, jumpForceToGroundMult = 5, slopJumpMult = 0.25, sprintJumpMult = 1.2, airDragMultiplier = 0.2, dragDampingC = 0.15, accDeltaTime = 8, rejectVelMult = 4, moveImpulsePointY = 0.5, camFollowMult = 11, fallingGravityScale = 2.5, fallingMaxVel = -20, wakeUpDelay = 200, 
// Floating Ray setups
rayOriginOffest = { x: 0, y: -capsuleHalfHeight, z: 0 }, rayHitForgiveness = 0.1, rayLength = capsuleRadius + 2, rayDir = { x: 0, y: -1, z: 0 }, floatingDis = capsuleRadius + floatHeight, springK = 1.2, dampingC = 0.08, 
// Slope Ray setups
showSlopeRayOrigin = false, slopeMaxAngle = 1, // in rad
slopeRayOriginOffest = capsuleRadius - 0.03, slopeRayLength = capsuleRadius + 3, slopeRayDir = { x: 0, y: -1, z: 0 }, slopeUpExtraForce = 0.1, slopeDownExtraForce = 0.2, 
// AutoBalance Force setups
autoBalance = true, autoBalanceSpringK = 0.3, autoBalanceDampingC = 0.03, autoBalanceSpringOnY = 0.5, autoBalanceDampingOnY = 0.015, 
...
...props }, ref) => {
    ...
    const gamepadConnect = (e) => { controllerIndex = e.gamepad.index; };
    ...
    const moveCharacter = (_, run, slopeAngle, movingObjectVelocity) => {
        ...
        if (actualSlopeAngle < slopeMaxAngle &&
         ...
         */
        moveAccNeeded.set((movingDirection.x *
            ...
            accDeltaTime, 0, (movingDirection.z *
            (maxVelLimit * (run ? sprintMult : 1) +
         ...
         */
        const characterRotated = Math.sin(characterModelIndicator.rotation.y).toFixed(3) ==
        ...
        if (!characterRotated) {
            moveImpulse.set(moveForceNeeded.x *
                turnVelMultiplier *
            ...
            slopeAngle === null || slopeAngle == 0 // if it's on a slope, apply extra up/down force to the body
                ...
                : movingDirection.y *
                    ...
                    (movingDirection.y > 0 // check it is on slope up or slope down
                        ? slopeUpExtraForce
                        : slopeDownExtraForce) *
                    (run ? sprintMult : 1), moveForceNeeded.z *
                turnVelMultiplier *
                (canJump ? 1 : airDragMultiplier) // if it's in the air, give it less control
            );
        }
        // If character complete turning, change the impulse quaternion default
            ...
            moveImpulse.set(moveForceNeeded.x * (canJump ? 1 : airDragMultiplier), slopeAngle === null || slopeAngle == 0 // if it's on a slope, apply extra up/down force to the body
                ? 0
                : movingDirection.y *
                    (movingDirection.y > 0 // check it is on slope up or slope down
                        ? slopeUpExtraForce
                        : slopeDownExtraForce) *
                    (run ? sprintMult : 1), moveForceNeeded.z * (canJump ? 1 : airDragMultiplier));
        }
    ...
    const autoBalanceCharacter = () => {
        ...
        dragAngForce.set((crossVecOnX.x < 0 ? 1 : -1) *
            autoBalanceSpringK * (bodyBalanceVecOnX.angleTo(vectorY))
            - characterRef.current.angvel().x * autoBalanceDampingC, (crossVecOnY.y < 0 ? 1 : -1) *
            autoBalanceSpringOnY * (modelFacingVec.angleTo(bodyFacingVecOnY))
            - characterRef.current.angvel().y * autoBalanceDampingOnY, (crossVecOnZ.z < 0 ? 1 : -1) *
     ...
     */
    const pointToMove = (delta, slopeAngle, movingObjectVelocity) => {
    ...
    useFrame((state, delta) => {
        ...
        if (controllerIndex !== null) {
            ...
            modelEuler.y = ((movingDirection) => movingDirection === null ? modelEuler.y : movingDirection)(getMovingDirection(gamepadKeys.forward, gamepadKeys.backward, gamepadKeys.leftward, gamepadKeys.rightward, pivot));
        }
        /**
        ...
        rayHit = world.castRay(rayCast, rayLength, true, null, null, 
        // I have no idea
        characterRef.current, characterRef.current, 
        ...
        if (rayHit && canJump) {
            if (rayHit.collider.parent()) {
                ...
                if (rayHitObjectBodyType === 0 || rayHitObjectBodyType === 2) {
                    ...
                    movingObjectVelocity.set(movingObjectLinvel.x +
                        objectAngvelToLinvel.crossVectors(movingObjectAngvel, distanceFromCharacterToObject).x, movingObjectLinvel.y, movingObjectLinvel.z +
                        objectAngvelToLinvel.crossVectors(movingObjectAngvel, distanceFromCharacterToObject).z).multiplyScalar(Math.min(1, 1 / massRatio));
        ...
        slopeRayHit = world.castRay(slopeRayCast, slopeRayLength, true, null, null, 
        // Still no idea
        characterRef.current, characterRef.current, 
        // this exclude with sensor collider
        ((collider) => !collider.isSensor()));
    ...
    return (_jsxs(RigidBody, { colliders: false, ref: characterRef, position: props.position || [0, 5, 0], friction: props.friction || -0.5, onContactForce: (e) => bodyContactForce.set(e.totalForce.x, e.totalForce.y, e.totalForce.z), onCollisionExit: () => bodyContactForce.set(0, 0, 0), userData: { canJump: false }, ...props, children: [_jsx(CapsuleCollider, { name: "character-capsule-collider", args: [capsuleHalfHeight, capsuleRadius] }), isModePointToMove &&
                _jsx(CylinderCollider, { ref: bodySensorRef, sensor: true, args: [capsuleHalfHeight / 2, capsuleRadius], position: [0, 0, capsuleRadius / 2], onIntersectionEnter: handleOnIntersectionEnter, onIntersectionExit: handleOnIntersectionExit }), _jsxs("group", { ref: characterModelRef, userData: { camExcludeCollision: true }, children: [_jsx("mesh", { position: [
                            rayOriginOffest.x,
                            rayOriginOffest.y,
                            rayOriginOffest.z + slopeRayOriginOffest,
                        ], ref: slopeRayOriginRef, visible: showSlopeRayOrigin, userData: { camExcludeCollision: true }, children: _jsx("boxGeometry", { args: [0.15, 0.15, 0.15] }) }), children] })] }));
});
